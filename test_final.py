...
def estimate_structural_roof_elevation(pts, ground_elev, bid):
    ...
    for z_test in sorted_z:
        ...
        if len(band_pts) > 0:
            ...
            # Robust criteria: a dominant structural roof tier must support a large area or fraction of the building
            if (point_ratio >= 0.15 or len(band_pts) >= 50) and (area_ratio >= 0.15 or band_area >= 50.0):
                # We return the max Z in this accepted band, which is z_test
                return float(z_test)
